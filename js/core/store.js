...
(function (global) {
  ...
  /* reset must not swap the object: every module captured `CBE.state` at load
     time, so replacing it would leave them reading the old, un-reset values */
  function reset() {
    var fresh = defaultState();
    var k;
    for (k in state) if (Object.prototype.hasOwnProperty.call(state, k)) delete state[k];
    for (k in fresh) if (Object.prototype.hasOwnProperty.call(fresh, k)) state[k] = fresh[k];
    save();
  }
